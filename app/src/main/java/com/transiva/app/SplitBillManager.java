...
public final class SplitBillManager {
    ...
    public SplitBillManager(Activity a, Listener l){this.a=a;this.listener=l;}
    public String sessionKey(){return sessionKey;} public int groupSize(){return groupSize;} public boolean ready(){return ready;}
    public void clear(){sessionKey="";groupSize=1;ready=false;stopPoll();changed();}
    public void start(int size,int expectedTotal,String context){
        ...
        for(int i=1;i<n;i++){EditText e=new EditText(a);e.setHint("Customer "+(i+1));e.setSingleLine(true);e.setInputType(InputType.TYPE_CLASS_TEXT);box.addView(e,new LinearLayout.LayoutParams(-1,56));fields.add(e);}
    ...
    private void create(int n,int total,String context,JSONArray targets){new Thread(()->{try{JSONObject p=new JSONObject().put("action","create").put("group_size",n).put("expected_total",Math.max(0,total)).put("context",context).put("targets",targets);JSONObject r=TransivaHttpRepository.postJson(a,ApiConfig.ROOT+"server/split_bill.php",p,15000);h.post(()->{if(!r.optBoolean("success")){toast(r.optString("message","Gagal mengirim undangan"));return;}sessionKey=r.optString("session_key","");groupSize=n;ready=false;changed();showStatus();});}catch(Exception e){h.post(()->toast("Gagal mengirim undangan: "+e.getMessage()));}}).start();}
    public void showStatus(){if(sessionKey.isEmpty()){toast("Belum ada sesi Split Bill");return;}fetch(true);}
    private void fetch(boolean show){new Thread(()->{try{JSONObject p=new JSONObject().put("action","status").put("session_key",sessionKey);JSONObject r=TransivaHttpRepository.postJson(a,ApiConfig.ROOT+"server/split_bill.php",p,12000);h.post(()->renderStatus(r,show));}catch(Exception e){h.post(()->{if(show)toast("Status Split Bill gagal dimuat");});}}).start();}
    private void renderStatus(JSONObject r,boolean show){if(!r.optBoolean("success")){if(show)toast(r.optString("message","Sesi Split Bill gagal"));return;}JSONObject s=r.optJSONObject("session");JSONArray ps=r.optJSONArray("participants");int remain=s==null?0:s.optInt("expires_in",0);boolean all=true;int accepted=0;if(ps!=null)for(int i=0;i<ps.length();i++){String st=ps.optJSONObject(i).optString("status");if("accepted".equals(st))accepted++;else all=false;}ready=all&&ps!=null&&ps.length()==groupSize;changed();
