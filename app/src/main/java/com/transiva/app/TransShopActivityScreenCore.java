...
class TransShopActivityScreenCore extends Activity {
    ...
    protected static final String PROFILE_URL = BASE_URL + "server/get_customer_profile.php";
    ...
    protected final CustomerFeatureRuntimeController featureRuntime = new CustomerFeatureRuntimeController(CustomerRealtimeCoordinator.Role.SHOP);
    ...
    protected void buildLayout() {
        ...
        TextView title = text("TransShop", 17, "#0B3A78", true);
        titleRow.addView(title, new LinearLayout.LayoutParams(0, -1, 1));

        modeText = text("Pilih toko / lokasi belanja", 10, "#64748B", false);
        ...
        pickupBtn = compactPointButton("●  Belanja di sini", "Belum dipilih", "#16A34A");
        deliveryBtn = compactPointButton("●  Tujuan", "Belum dipilih", "#EF4444");

        pointRow.addView(pickupBtn, new LinearLayout.LayoutParams(-1, -1));

        pickupText = text("Belanja di: belum dipilih", 9, "#334155", false);
        ...
        googleMapInput.setHint("Link Google Maps lokasi belanja (opsional)");
        ...
        driverAvailabilityText = text("Tidak ada driver TransShop yang online", 10, "#B91C1C", true);
    ...
    protected void loadDeliveryLocation() {
        if (userId <= 0) {
    ...
    protected void useGoogleMapLink() {
        ...
        if (link.length() == 0) {
            toastDialog("Masukkan link Google Maps lokasi belanja terlebih dahulu.");
            return;
        }
        hideKeyboard();
        setLoading(true);
        featureRuntime.newThread(() -> {
            ...
            try {
                ...
                mainHandler.post(() -> {
                    ...
                    pickupLat = c[0]; pickupLng = c[1]; pickupAddress = "Mencari alamat lokasi belanja...";
                    pickupText.setText("Belanja di: " + pickupAddress);
                    pickupBtn.setText("●  Belanja di sini\nMencari alamat...");
    ...
    protected String reverseAddress(double lat, double lng) {
        ...
        try {
            ...
            conn = CustomerApiClient.open(this, url.toString());
    ...
    protected void createOrder() {
        ...
        if (!validCoord(pickupLat, pickupLng)) {
            toastDialog("Pilih lokasi belanja terlebih dahulu.");
            return;
        }

        if (!validCoord(deliveryLat, deliveryLng)) {
            toastDialog("Alamat delivery akun belum valid. Buka menu Akun dan tetapkan alamat delivery.");
            return;
        }

        String shoppingList = noteInput == null ? "" : noteInput.getText().toString().trim();
        if (shoppingList.isEmpty()) {
            toastDialog("Tulis daftar barang yang ingin dibelanjakan terlebih dahulu.");
            return;
        }
        ...
        featureRuntime.newThread(() -> {
            try {
                ...
                pickup.put("address", firstNonEmpty(pickupAddress, "Lokasi Belanja"));
                ...
                payload.put("order_type", "TransShop");
                payload.put("driver_type", "bike");
                payload.put("service_type", "TransShop");
                payload.put("service_name", "TransShop");
                payload.put("price_mode", "server");
                payload.put("pickup", pickup);
                payload.put("delivery", delivery);
                payload.put("pickup_address", firstNonEmpty(pickupAddress, "Lokasi Belanja"));
    ...
    protected void handleOrderResult(JSONObject res) {
        ...
        getSharedPreferences("transiva", MODE_PRIVATE).edit()
                ...
                .putString("active_order_type", "TransShop")
                .putString("active_driver_type", "bike")
                .putString("active_service_name", "TransShop")
                ...
                .putString("pickup_address", firstNonEmpty(pickupAddress, "Lokasi Belanja"))
        ...
        try {
            ...
            i.putExtra("active_order_type", "TransShop");
            ...
            i.putExtra("pickup_address", firstNonEmpty(pickupAddress, "Lokasi Belanja"));
            i.putExtra("delivery_address", firstNonEmpty(deliveryAddress, "Lokasi Pengantaran"));
            startActivity(i);
            finish();
        } catch (Exception e) {
            toastDialog("Order TransShop berhasil dibuat. ID: " + orderId);
        }
    }

    protected void requestPaymentQuote() {
        requestVisibleOsrmRoute();
        if (!validCoordinate(pickupLat, pickupLng)
                || !validCoordinate(deliveryLat, deliveryLng)) {
            if (paymentSummaryText != null) {
                paymentSummaryText.setText("Pilih lokasi belanja; alamat pengantaran mengikuti alamat delivery akun");
            }
    ...
    protected int jsonInt(JSONObject json, String key, int fallback) {
        return CustomerJsonValues.intValue(json, key, fallback);
    }

    protected double jsonDouble(JSONObject json, String key, double fallback) {
    ...
    protected boolean validCoordinate(double latitude, double longitude) {
    ...
    protected void updateModeUI() {
        ...
        if (mapView != null) {
            if (routeComplete) {
                mapView.showOrderAction(true, "PESAN TRANSHOP");
            } else {
                mapView.setSelectionMode("pickup");
                mapView.showCenterPin(true);
            }
        }
    }

    protected void requestVisibleOsrmRoute() {
    ...
    protected boolean validCoord(double lat, double lng) {
        return CustomerGeoMath.valid(lat, lng);
    }
    protected boolean isDriverBusy(JSONObject driver) {
    ...
    private void resetOrderButton() { ordering = false; setLoading(false); orderBtn.setEnabled(true); orderBtn.setText("🛍️ Pesan TransShop"); }
