...
public class TransivaFamilyActivity extends Activity {
    ...
    private void build(){
        ...
        ScrollView sc=new ScrollView(this); list=new LinearLayout(this); list.setOrientation(LinearLayout.VERTICAL); sc.addView(list); body.addView(sc,new LinearLayout.LayoutParams(-1,0,1)); root.addView(body,new LinearLayout.LayoutParams(-1,0,1)); setContentView(root); CustomerAppSettings.apply(this);
    }
    ...
    private void addMember(JSONObject x){
        ...
        LinearLayout tools=new LinearLayout(this); Button live=smallButton("Pantau"), pay=smallButton("Bayar"), edit=smallButton("Edit"), del=smallButton("Hapus"); tools.addView(live,new LinearLayout.LayoutParams(0,dp(40),1)); LinearLayout.LayoutParams plp=new LinearLayout.LayoutParams(0,dp(40),1);plp.setMargins(dp(6),0,0,0);tools.addView(pay,plp); LinearLayout.LayoutParams elp=new LinearLayout.LayoutParams(0,dp(40),1);elp.setMargins(dp(6),0,0,0);tools.addView(edit,elp); LinearLayout.LayoutParams dlp=new LinearLayout.LayoutParams(0,dp(40),1);dlp.setMargins(dp(8),0,0,0);tools.addView(del,dlp);LinearLayout.LayoutParams tlp=new LinearLayout.LayoutParams(-1,-2);tlp.setMargins(0,dp(8),0,0);c.addView(tools,tlp);
        live.setOnClickListener(v->openFamilyLive(id,name)); pay.setOnClickListener(v->toggleFamilyPayment(id,name)); motor.setOnClickListener(v->openRide(TransRideActivity.class,id,name)); car.setOnClickListener(v->openRide(PassengerCarActivity.class,id,name)); edit.setOnClickListener(v->edit(id,name,phone,rel)); del.setOnClickListener(v->confirmDelete(id,name)); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2);lp.setMargins(0,0,0,dp(10));list.addView(c,lp);
    }
    ...
    private void openFamilyLive(int id,String name){Intent i=new Intent(this,FamilyLiveActivity.class);i.putExtra("family_member_id",id);i.putExtra("family_member_name",name);startActivity(i);}
    private void toggleFamilyPayment(int id,String name){try{JSONObject o=new JSONObject();o.put("action","toggle_shared_payment");o.put("member_id",id);sendV2(o,"Pengaturan pembayaran Family untuk "+name+" diperbarui.");}catch(Exception ignored){}}
    private void sendV2(JSONObject o,String ok){TransivaNetworkExecutor.execute(()->{try{JSONObject r=post("https://transiva.my.id/server/customer_family_v2.php",o);runOnUiThread(()->Toast.makeText(this,r.optString("message",ok),Toast.LENGTH_LONG).show());}catch(Exception e){runOnUiThread(()->Toast.makeText(this,"Family 2.0 belum tersambung ke server.",Toast.LENGTH_SHORT).show());}});}
    ...
    private Button button(String text,boolean primary){Button b=new Button(this);b.setText(text);b.setAllCaps(false);b.setTextSize(13);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setTextColor(Color.parseColor(themeColor(primary?"#FFFFFF":"#0B6DD9")));b.setBackground(primary?bg("#0878F9",16):bgStroke("#F0F7FF","#CDE4FF",15,1));return b;} private Button smallButton(String text){Button b=button(text,false);b.setTextSize(12);return b;}
    private TextView tx(String s,int z,String color,boolean bold){TextView v=new TextView(this);v.setText(s);v.setTextSize(z);v.setTextColor(Color.parseColor(themeColor(color)));if(bold)v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);return v;} private GradientDrawable bg(String color,int radius){GradientDrawable g=new GradientDrawable();g.setColor(Color.parseColor(themeColor(color)));g.setCornerRadius(dp(radius));return g;} private GradientDrawable bgStroke(String fill,String stroke,int radius,int width){GradientDrawable g=bg(fill,radius);g.setStroke(dp(width),Color.parseColor(themeColor(stroke)));return g;} private boolean isDark(){return CustomerAppSettings.isDarkMode(this);}
